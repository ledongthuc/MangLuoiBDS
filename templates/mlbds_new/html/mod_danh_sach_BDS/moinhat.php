...
<form action="<?php echo curPageURL(); ?>" name="formclause" method="get">
	<select class="input-s" name="clause" onchange="changeOrder(this.options[this.selectedIndex].value)" style="width: 182px">
			<option value="ngay_chinh_sua" <?php if(isset($_GET['clause'])&&($_GET['clause'])=='ngay_chinh_sua') echo "selected='selected'"?> >Ngày đăng</option>
...
<div class='items-icon'>
	...
	<div class='f-r-tm'>
		<div style='float:left;width:496px'>
				...
				<span class='its' style="float:right">
		...
		<div class='clear'>
				<ul class='clear'>
					...
					<li class="icon-map">
						<img style="cursor: pointer;" onclick="loadMap(<?php echo $item['kinh_do'] ?>,<?php echo $item['vi_do']?>,'<?php echo $item['dia_chi'] ?>','<?php echo JURI::base()?>')" src="<?php echo JURI::base()?>images/icon-map.png" alt="Bản đồ" title="Xem bất động sản trên bản đồ" />						
					</li>
				</ul>
				<ul class='clear'>
					<li class='li-title'>
