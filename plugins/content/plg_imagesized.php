...
/******************Plugin Parameters*******************************/   
		...
		$fp_margin_top = (int) $this->params->get( 'fp_margin_top', '0' );
		$fp_margin_bottom = (int) $this->params->get( 'fp_margin_bottom', '0' );
		$fp_margin_left = (int) $this->params->get( 'fp_margin_left', '0' );
		$fp_margin_right = (int) $this->params->get( 'fp_margin_right', '0' );
	...
	if ($have_images)	
	{
		  ...
		  foreach ($plg_matches[0] as $plg_match) 
		  {
			...
			if(in_array($view[0],$viewtype)) {//frontpage, catblog, sectionblog
				...
				$showbottext = $this->remove_imagelink_info($showbottext,'margin-top',$fp_margin_top);
				$showbottext = $this->remove_imagelink_info($showbottext,'margin-bottom',$fp_margin_bottom);
				$showbottext = $this->remove_imagelink_info($showbottext,'margin-left',$fp_margin_left);
				$showbottext = $this->remove_imagelink_info($showbottext,'margin-right',$fp_margin_right);
				$showbottext = $this->change_imagelink_info($showbottext,'style','width:'.$widthm.'px; '.
				'height:'.$heightm.'px; margin:'.$fp_margin_top.'px '.$fp_margin_right.'px '.$fp_margin_bottom.'px '.$fp_margin_left.'px; float:left;'.'padding:1px; border: 1px solid #f0f0f0;');
				...
				//them code cho corner image
				//$showbottext = $this->change_imagelink_info($showbottext,'class','corner iradius8 ishadow43');						
				//ket thuc
