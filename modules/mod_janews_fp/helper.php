...
if(!defined('_JA_NEWS_')){
	...
	class modJaNewsHelper
	{
		...
		function getHLNews($items,&$params) {
			
//minh viet ne
			$showby=$params->get('showby');
			$showbycondition='';
			if($showby == 'section')
			{
				...
				if ($secid)
				{
					...
					$showbycondition = ' AND (s.id=' . implode( ' OR s.id=', $ids ) . ')';
				}
			}
			if($showby == 'category')
			{
				...
				if ($catid)
				{
					$ids = explode( ',', $catid );
					JArrayHelper::toInteger( $ids );
					$showbycondition = ' AND (cc.id=' . implode( ' OR cc.id=', $ids ) . ')';
				}
			}	
			//echo $query;
			//echo '<br>xxxxx: '.$params->get('showby');
// ket thuc mi code
