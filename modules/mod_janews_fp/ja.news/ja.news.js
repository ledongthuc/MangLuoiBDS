/*------------------------------------------------------------------------
# JA Dravity for joomla 1.5 - Version 1.4 - Licence Owner JA130162
# ------------------------------------------------------------------------
...
JA_NewsHeadline = new Class ({
	...
	animfirst: function (){
		...
		if($('jahl-prev-title')) {
			if ($('jahl-prev-title').getChildren()) $('jahl-prev-title').getChildren().each(function(el){el.remove();});
			this.elements[this.getPrev()].getElement('.ja-newstitle').clone().inject($('jahl-prev-title'));
		}
		if($('jahl-next-title')) {
			if ($('jahl-next-title').getChildren()) $('jahl-next-title').getChildren().each(function(el){el.remove();});
			this.elements[this.getNext()].getElement('.ja-newstitle').clone().inject($('jahl-next-title'));
		}
