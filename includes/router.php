...
class JRouterSite extends JRouter
{
	...
	function _parseSefRoute(&$uri)
	{
		...
		else if ( substr($route, 0, 4) == 'duan' || substr($route, 0, 6) == 'nhadat' ) // for u_re
		{
			$segments	= explode('/', $route);
			//$route      = str_replace('component/'.$segments[1], '', $route);

			$vars['option'] = 'com_u_re';
			$vars['Itemid'] = $segments[ count($segments) - 2 ];
			$vars['id'] = $segments[ count($segments) - 1 ];
		}
		// dang tin front end
		else if ( substr($route, 0, 4) == 'save' )
		{
			$segments	= explode('/', $route);
			//$route      = str_replace('component/'.$segments[1], '', $route);
		...
		if(!empty($route) && isset($this->_vars['option']) )
		{
			...
			if ( $segments[0] == 'component' ) // su dung cho truong hop com_u_re
			{
	...
	function _buildSefRoute(&$uri)
	{
		...
		// ko build phan quan li tin front end
		if ( isset( $query['view'] ) && $query['view'] == 'manage' && isset( $query['option'] ) && $query['option'] == 'com_u_re')
		{
			return;
		}
		
		// khong build phan doc man
		if ( isset( $query['option'] ) && $query['option'] == 'com_docman' )
		{
			return;
		}
		
		// khong build phan history
		if ( isset( $query['option'] ) && $query['option'] == 'com_history' )
		{
			return;
		}
		
		
		// khong build phan user
		if ( isset( $query['option'] ) && $query['option'] == 'com_user' )
		{
			return;
		}
		// khong build phan user
		if ( isset( $query['option'] ) && $query['option'] == 'com_hengio' )
		{
			return;
		}
		...
		// Chau: fix hiden menu item id: com google
		if ( !empty($query['Itemid']) && $query['Itemid'] != '192' )
		{
			unset($query['Itemid']);
		}
