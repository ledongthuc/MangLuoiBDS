...
defined('_JEXEC') or die();
	...
	function write_text_file($in_filename, $string)
	{
		$arr = explode("/",$in_filename);
		$directory = '';
...
class plgSystemOSOLCaptcha extends JPlugin
{
	...
	function create_imageAdv(){
		...
		//global $key_string;
		//$key_string = $this->keystring;
		//$GLOBALS['key_string'] = $this->keystring;
		// Chau add global var
		//global $u_reGlobalConfig;
		//$u_reGlobalConfig['keyString'] = $this->keystring;
		//$keyString = $this->keystring;

		$security_code = $this->keystring;
		
		// write security code in file
		...
		//$file  = new ReadSecurityCode();
		$ip = $_SERVER['REMOTE_ADDR'];
		$ip = str_replace('.', '_', $ip);
		
		$fileName = $rootPath . '/images/security_code/sercu_'.$ip.'.txt';
		
		$currentSession =  & JFactory::getSession() ;//&JSession::getInstance('none',array()); 
		$currentSession->set('file_name', $fileName);
		
		write_text_file($fileName, $this->keystring);
		$currentSession =  & JFactory::getSession() ;
		$currentSession->set('write', '1');
	...
	function confirm( $word,$instanceNo='' )
	{
		...
		$_SESSION['tmp']=$_POST;
		$_SESSION['tmp']['captcha'] = "Captcha sai, vui lòng nhập lại. ";
		if($_SESSION['tmp']['anh_tmp']!=''){
			$_SESSION['tmp']['captcha'] .= "Bạn vui lòng chọn lại hình ảnh cho bất động sản";
		}
		unset($_SESSION['tmp']['password']);
		unset($_SESSION['tmp']['password2']);
		unset($_SESSION['tmp']['osolCatchaTxt']);
		unset($_SESSION['tmp']['osolCatchaTxtInst']);
		$currentSession = & JFactory::getSession() ;
...
//				echo "324";
			//	return;
				...
				//$mainframe->redirect('index.php?option=com_jea&view=manage&layout=form&Itemid=6&lang=vi');
			}
			else
			{
				$this->botscoutCheck();
		...
		function GetCapthcaHTML($vertical = false)
		{
			...
			/*
			if($_SESSION['captchasai']){
				$reload = "reloadCapthcha0(0);";
			}
			else{
				$reload = "";
			}
			*/
			if($_GET['Itemid']==19 || $_GET['Itemid']==224){
				$sty="style='margin-left:4px'";
			}
			return ("
					<div class=\"osolCaptchaBlock\" ".$sty.">
  ...
  <tr>
	...
	<a href=\"javascript:;\" target=\"blank\" style=\"float: left;font-size: 10px;\" >
	<img id=\"captchaCode".$GLOBALS['totalCaptchas']."\" src=\"".JURI::base()."index.php?showCaptcha=True&amp;instanceNo=".$GLOBALS['totalCaptchas']."\" alt=\"Captcha mạng lưới bất động sản\" /> 
	</a>
	...
	<script language=\"javascript\" type=\"text/javascript\">
       ...
       <a href=\"#\" style=\"vertical-align: top;\" onclick=\"reloadCapthcha{$GLOBALS['totalCaptchas']}(".$GLOBALS['totalCaptchas'].");return false;\" >".JText::_('Cick vào đây để thay đổi mã an toàn')."</a>
    </label></td>
	".($vertical?"</tr><tr>":"")."
	</tr>
	<tr>
	...
	<input type=\"hidden\" name=\"osolCatchaTxtInst\" id=\"osolCatchaTxtInst\"  value=\"".$GLOBALS['totalCaptchas']."\" />
