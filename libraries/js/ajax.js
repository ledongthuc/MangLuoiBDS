...
//	alert(address);
 ...
 curpage=1;
 function ref(page,totalpage) {
		var item=50;   
		var total=totalpage;
		var items=(page-1)*item+1; 
		var itemsnext=items+item-1;
		if (itemsnext>totalpagex) itemsnext=totalpagex;
...
function waitstart(){
 ...
 function loadOrder(status,fkey,forder) {
	 var asty = $('#tknc').attr('act');
	 if(asty == '1'){
		 showBasicSearch();
	 }
	waitstart();
	gurl=rurl+'&fkey='+fkey+'&forder='+forder; 
	var url=gurl+'&page=1';
	
	url=url.replace(/,/g,'-'); 
	for (i in lmaker) {
		lmaker[i].setMap(null); 
    }
	lmaker=[];
	$('.active_page').attr('class','page_next');   
	$('#firstitem').attr('class','active_page');
    document.getElementById('sortstatus').innerHTML=status;
	hideSort();
 ...
 function getAjaxPaging(currentPage,totalPage,numdisplay ,url,element) 
 {
 	if ( currentPage > totalPage )
 	{
 		currentPage = totalPage;
	...
	ref(currentPage,totalPage);  
	 
	if (curpage==currentPage) return; else curpage=currentPage;
	$('.active_page').attr('class','page_next');  
	$(element).attr('class','active_page');	
	url = gurl+'&page='+currentPage; 
	url=url.replace(/,/g,'-');
	// Clear All Marker in Google Maps  
	for (i in lmaker) {
		lmaker[i].setMap(null); 
    }
	lmaker=[];
