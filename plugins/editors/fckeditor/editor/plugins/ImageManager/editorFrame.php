...
?>
	...
	if (processedAction != '')
	{
		...
		//setTimeout( "window.top.toggle( '" + processedAction + "')", 500 );
	}
...
<?php
	...
	foreach($IMConfig['watermarks'] as $watermark)
	{
		...
		$watermarkInfo = @getImageSize($IMConfig['root_dir'] . $watermark);
		...
		if ($watermarkInfo[0] < $imageInfo['width'] && $watermarkInfo[1] < $imageInfo['height'] && $watermarkInfo[0] != '')
		{
			...
			echo "watermarkBox.options[watermarkBox.options.length - 1].setAttribute('fullPath', '" . urlencode($IMConfig['root_dir'] . $watermark) . "');";
			...
			echo $filename . "Preload.src = '" . $IMConfig['base_root'] . $watermark . "';";
		}
...
<?php if(count($imageInfo) > 0 && is_file($imageInfo['fullpath'])) { ?>
		...
		<?php if (count($IMConfig['watermarks']) > 0) { ?>
			<img name="floater" id="floater" style="width: 150px; height: 150px; behavior: url('assets/pngbehavior.htc'); position: absolute" 
			src="<?php echo $IMConfig['base_url'] . $IMConfig['watermarks'][0]; ?>" />
		<?php } ?>
		</div>
