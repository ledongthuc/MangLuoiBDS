...
/* add script for admin google map */
    ...
    var defaultPosition = new google.maps.LatLng(106.70015,10.777746);
    var marker;
...
function valid_dathangF(form)
{
	...
	if(e['phone'].value == "" )
	{
		error +="*điện thoại nhập không được để trống\n\n";
		check = false;
	}
	if(e['content'].value == "" )
	{
...
function checkInternationalPhone(strPhone){
	...
	var phoneReg = /^(0|84)((2(([0,2,5-7,9])|(1[0,1,8,9])|([3,4][0,1]))[2-9][0-9]{5,6})|(3(([0,1,3,6-9])|([2,5][0,1]))[2-9][0-9]{5,6})|(5(([0,1][0,1])|([2-9]))[2-9][0-9]{5,6})|(6((5[0,1])|([0-4,6-9]))[2-9][0-9]{5,6})|(7(([1,8][0,1])|([0,2-7,9]))[2-9][0-9]{5,6})|([4,8][2-9][0-9]{7}))$/;
	var mobileReg = /^0((9[0-9])|(12[0-9])|(16[0-9])|199)[0-9]{7}$/;
	if ( mobileReg.test( strPhone ) )
	{
		return true;
	}
	else if ( phoneReg.test( strPhone ) )
	{
		return true;
	}
	// return ( phoneReg.test( strPhone ) && mobileReg.test( strPhone ) );
	return false;
	/*
...
function valid_regisF(form,task)
 {
	...
	var e = form.elements;
	/* Your validation code. */
	e.save_published.disabled = true;
	var check='true';
	...
	{
		err+='- Mã an toàn không được để trống\n';
		form.osolCatchaTxt.style.borderColor="#FF0000";form.osolCatchaTxt.focus();
		check = 'false';
	}else{
		form.osolCatchaTxt.style.borderColor='#96A6C5';
	}
	...
	*/
	...
	//	return false;
	//}
	if(e['emailFail'].value == '1' )
	{
		err+='- Email này đã được đăng ký vui lòng nhập email khác\n';
		form.email.style.borderColor="#FF0000";form.email.focus();
		check = 'false';
	}
	...
	if(e['phone'].value == "" )
	{
		...
		form.phone.style.borderColor="#FF0000";form.phone.focus();
		check = 'false';
	}
	else{ 
		...
		if(!checkInternationalPhone(e['phone'].value)){
			...
			form.phone.style.borderColor='#96A6C5';
		}
	}
	if (checkEmail == "")
	{
		...
		if(!validate_Email_Address(checkEmail)){
			...
			form.email.style.borderColor='#96A6C5';
		}
	...
	if(e['name'].value == "" )
	{
		err+='- Tên người dùng không được để trống\n';
		form.name.style.borderColor="#FF0000";form.name.focus();
		check = 'false';
	}else{
		form.name.style.borderColor='#96A6C5';
	}
	if(e['password'].value == "" )
	{
		err+='- Mật khẩu không được để trống\n';
		form.password.style.borderColor="#FF0000";
		form.password2.style.borderColor="#FF0000";form.password.focus();
		check = 'false';
	}
	else{
		if(e['password'].value != e['password2'].value) 
		{
			...
			form.password.style.borderColor="#FF0000";form.password.focus();
			check = 'false';
		}else{
			form.password.style.borderColor='#96A6C5';
			form.password2.style.borderColor='#96A6C5';
		}		
	}
	if(check == 'false')
	{
		...
		alert(err);
		e.save_published.disabled = false;
		/*
		if(err['phone']!=null){
			alert(err['phone']);
...
function valid_regisFTYC(form1)
{
...
function submitForm( userlogin, userpro , published, re_link, lang)
{
	...
	var form = document.adminForm;
	form.save_published.disabled = true;
	...
	{	
		...
		if(userlogin==''||userlogin=='noregister'){
			/*if(form.email_vl.value==''){
				err += '- Vui lòng nhập email người liên hệ\n';
			...
			if(form.email_vl.value!=''){
				if(!validate_Email_Address(form.email_vl.value)){
					err += '- Email người liên hệ không hợp lệ\n';
					form.email_vl.style.borderColor="#FF0000";
					form.email_vl.focus();
					check =  'false';
				}else{
					form.email_vl.style.borderColor='#96A6C5';
				}
			}
			if(form.phone_vl.value==''){
				...
				if(!checkInternationalPhone(form.phone_vl.value)){
					err+='- Số điện thoại không hợp lệ\n';
					form.phone_vl.style.borderColor="#FF0000";form.phone_vl.focus();
					check = 'false';
				}else{
					form.phone_vl.style.borderColor='#96A6C5';
				}
			}
			if(form.name_vl.value==''){
				err += '- Vui lòng nhập tên người liên hệ\n';
				form.name_vl.style.borderColor="#FF0000";
				form.name_vl.focus();
				check =  'false';
			}else{
				form.name_vl.style.borderColor='#96A6C5';
			}
		}
		if(Validate_Number(form.dien_tich_su_dung.value)===false){
			err += '- Vui lòng nhập số diện tích sử dụng\n';
			form.dien_tich_su_dung.style.borderColor="#FF0000";
			form.dien_tich_su_dung.focus();
			check =  'false';
		}else{
			form.dien_tich_su_dung.style.borderColor='#96A6C5';
		}
		if(form.dien_tich_su_dung.value==''){
			err += '- Vui lòng nhập diện tích sử dụng\n';
			form.dien_tich_su_dung.style.borderColor="#FF0000";
			form.dien_tich_su_dung.focus();
			check =  'false';
		}else{
		...
		if(form.dia_chi.value==''){
			...
			form.dia_chi.style.borderColor='#96A6C5';
		}
		if(check == 'false'){
			form.save_published.disabled = false;
			alert(err);			
		}else{
			var duAnEle = document.getElementById('du_an_id');
			if ( duAnEle.selectedIndex > 0 )
			{
				form.du_an_text_value.value = duAnEle.options[duAnEle.selectedIndex].text;
			}
				...
				if (xmlHttp.readyState == 4)  { 
				    if (xmlHttp.status == 200) {
						try {
							...
							if(response == ''){
								alert("Thông tin đăng nhập không chính xác hoặc tài khoản chưa tồn tại, bạn vui lòng kiểm tra lại!");
								form.username.style.borderColor="#FF0000";
								form.username.focus();
								return;
							}
							...
							var usertype =	data[4];
							var name 	 =	data[5];
							if ( document.getElementById('phone_vl').value == "" )
							{
								document.getElementById('phone_vl').value = phone;
							}
							if ( document.getElementById('name_vl').value == "" )
							{
								document.getElementById('name_vl').value = name;
							}
							if ( document.getElementById('email_vl').value == "" )
							{
								document.getElementById('email_vl').value = email;
							}
							if ( document.getElementById('customer').value == "0" )
							{
...
function getDataLoginTYCBDS(form1,url,sql,listThongTinThemValue1, array_of_checked_values){
	var form = document.forms[form1];
	var e = form.elements;
	var username = e['username'].value;	
	var password = e['password'].value;	
//	var check = 'true';
	...
	var address= url+"checklogin.php?username=" + username + "&password=" + password;
	if(xmlHttp)
	{
		try
		{
				...
				if (xmlHttp.readyState == 4)  { 
				    if (xmlHttp.status == 200) {
						...
						//		check = 'false';
							...
							var iduser	 =	data[0];
							var email1	 =	data[3];
							//checkYeuCauBDS(iduser,username,password,url);
							form.customer.value = iduser;
							form.email.value	= email1;
							SubmitYCBDS(form1,sql,listThongTinThemValue1,array_of_checked_values);
						}
						catch(e){
							alert("Error reading the response: " + e.toString());
						}
...
function checkYeuCauBDS(userid,username,pass,url){	
	var address= url+"check_yeu_cau_bds.php?userid=" + userid + "&username="+username+"&password="+pass;
	if(xmlHttp)
	{
		try
		{
				...
				if (xmlHttp.readyState == 4)  { 
				    if (xmlHttp.status == 200) {
						try {
							var response = xmlHttp.responseText;
							if(response != ''){
								alert("Bạn đã tạo yêu cầu bất động sản");
							}							
						}
						catch(e){
							alert("Error reading the response: " + e.toString());
...
function validateForm( form ,lang )
{
	...
	{
		ms += '- Bạn phải chọn loại tin đăng\n';
	...
	if (Validate_Number(form.dien_tich_khuon_vien_rong.value)===false){
		ms += "Vui lòng nhập số cho chiều rộng\n";
	...
	if (Validate_Number(form.dien_tich_khuon_vien_dai.value)===false){
		ms += "Vui lòng nhập số cho chiều dài\n";
		countMessage++;
	...
	if (Validate_Number(form.phong_ngu.value)===false){
		ms += "Vui lòng nhập số cho phòng ngủ\n";
	...
	if (Validate_Number(form.phong_khac.value)===false){
		ms += "Vui lòng nhập số cho phòng khác\n";
	...
	if (Validate_Number(form.phong_tam.value)===false){
		ms += "Vui lòng nhập số cho phòng tắm\n";
