...
<form name="adminForm" id="adminForm" action="" method="post">
	<ul class='adminbutton'>
			...
			<input type="text" name="search" class='input-s1' 
				onclick="if(this.value=='Từ khóa tìm kiếm') this.value=''" 
	...
	<li>	
	    <select class='input-s5' name="published" onchange="adminForm.submit()">
...
//		}
		...
		?>
			...
			<th class="ref" width="80px">
					...
					<?php echo JText::_('DATE_MODIFIND')?>
			</th>	
...
<?php foreach ($this->rows as $k => $row): $altrow = $altrow ? 0 : 1 ?>
		<tr class="row<?php echo $altrow ?>" >
			...
			<td class="daytin">	 
				<input type="button" class="sb_ck3" onclick="<?php if ($row['hien_thi_ra_ngoai']) { ?> hen_gio_non_user(<?php echo $row['id'] ?>)" <?php } else { ?> alert('Tin này chưa được duyệt. Bạn không thể đẩy,đánh dấu,nổi bật tin này !');" <?php } ?> value="ĐẨY | ĐÁNH DẤU | NỔI BẬT TIN NÀY"> 		
			</td>	  
			<!--<td class="published">				
