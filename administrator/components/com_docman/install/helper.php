...
class DMInstallHelper 
{
    ...
    function copyFiles()
    {
		...
		JFolder::copy(
			JPATH_ADMINISTRATOR.DS.'components'.DS.'com_docman'.DS.'ext'.DS.'plugins'.DS.'editors-xtd'.DS.'doclink'.DS, 
        	JPATH_ROOT.DS.'plugins'.DS.'editors-xtd'.DS.'doclink'
		);
    ...
    function insertInDb()
    {
    	...
    	{
    		$query = "INSERT INTO `#__plugins`  (`name`, `element`, `folder`, `access`, `ordering`, `published`, `iscore`, `client_id`, `checked_out`, `checked_out_time`, `params`)"
    				."\n VALUES('DOCLink', 'doclink', 'editors-xtd', 0, 0, 0, 0, 0, 0, '0000-00-00 00:00:00', '')";
			$database->setQuery($query);
			$database->query();
    	}
    	...
    	if(!$database->loadResult())
    	{
    		self::_insertModule(array('title'=>'Latest docs', 'position'=>'dmcpanel', 'module'=>'mod_docman_latest', 'showtitle'=>1, 'params'=>'', 'ordering'=>0, 'published'=>1, 'client_id'=>1));
    	}
    	...
    	if(!$database->loadResult())
    	{
			self::_insertModule(array('title'=>'Top docs', 'position'=>'dmcpanel', 'module'=>'mod_docman_top', 'showtitle'=>1, 'params'=>'', 'ordering'=>1, 'published'=>1, 'client_id'=>1));
    	}
    	...
    	if(!$database->loadResult())
    	{
			self::_insertModule(array('title'=>'Latest logs', 'position'=>'dmcpanel', 'module'=>'mod_docman_logs', 'showtitle'=>1, 'params'=>'', 'ordering'=>2, 'published'=>1, 'client_id'=>1));
    	}
    	...
    	if(!$database->loadResult())
    	{
			self::_insertModule(array('title'=>'Top docs', 'position'=>'dmcpanel', 'module'=>'mod_docman_top', 'showtitle'=>1, 'params'=>'', 'ordering'=>3, 'published'=>1, 'client_id'=>1));
    	}
    	...
    	if(!$database->loadResult())
    	{
			self::_insertModule(array('title'=>'Top docs', 'position'=>'dmcpanel', 'module'=>'mod_docman_top', 'showtitle'=>1, 'params'=>'', 'ordering'=>4, 'published'=>1, 'client_id'=>1));
    	}
    	...
    	if(!$database->loadResult())
    	{
    		self::_insertModule(array('title'=>'DOCman Category', 'position'=>'left', 'module'=>'mod_docman_catdown', 'showtitle'=>1, 'params'=>''));
    	}
    	...
    	if(!$database->loadResult())
    	{
			self::_insertModule(array('title'=>'DOCman Latest Downloads', 'position'=>'left', 'module'=>'mod_docman_latestdown', 'showtitle'=>1, 'params'=>''));
    	}
    	
   	 	$query = "SELECT id FROM #__modules WHERE module = 'mod_docman_listerdown'";
    	$database->setQuery($query);
    	if(!$database->loadResult())
    	{
			self::_insertModule(array('title'=>'DOCman Category', 'position'=>'left', 'module'=>'mod_docman_catdown', 'showtitle'=>1, 'params'=>''));
    	...
    	if(!$database->loadResult())
    	{
			self::_insertModule(array('title'=>'DOCman Most Downloaded', 'position'=>'left', 'module'=>'mod_docman_mostdown', 'showtitle'=>1, 'params'=>''));
    	}
    ...
    function deleteFromDb()
     {
    	...
    	"DELETE FROM #__modules WHERE module = 'mod_docman_listerdown'",
