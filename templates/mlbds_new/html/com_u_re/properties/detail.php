...
{
?>
	<form action="<?php echo JRoute::_('&task=save') ?>" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data"  onsubmit="return valid_regisF(this);"/>
<?php
}
?>
<?php		
...
<tr>
	<td height="30" align='right' valign='top'>
		<label id="usernamemsg" for="username">
			<?php echo JText::_( 'User name' ); ?>: <span class='red'>*</span> 
		</label>
	</td>
	<td>
		<input type="text" id="username" name="username" size="30" value="<?php if(isset($_SESSION['tmp'])) echo $_SESSION['tmp']['username'];?>" maxlength="25" onBlur="KiemTraUsername(this.value,'msgErrUsername')" /> <div id="msgErrUsername" class="errouser"><input type="hidden" name="userFail" id="userFail" value="0"  /><i style="color:black">Tên đăng nhập chỉ bao gồm ký tự từ a-z, 0-9</i></div>
	</td>
...
<tr>
	...
	<td>
		<input type="text" id="email" name="email" size="30" value="<?php if(isset($_SESSION['tmp'])) echo $_SESSION['tmp']['email'];?>"  maxlength="100" onchange="setValue(this.value,'email_vl')"  onBlur="KiemTraEmail(this.value,'msgErrEmail','<?php echo JURI::base() ?>')"/><div id="msgErrEmail" class="errouser"><input type="hidden" id="emailFail" value="0"  /></div>
	</td>
</tr>
<tr>
	<td height="30" align='right'>
...
<tr>
	...
	<td style="padding-left: 30px;">
		<span class="clear check" style='margin-bottom:5px;'>
						<input type="checkbox" name="speak_english1" id="speak_english1" onchange="setCheckbox(speak_english1,'speak_english')"> 
							Can you speak English?
		</span>
		<span class="clear check" style="margin-bottom:5px;">
					<input type="checkbox" name="chinh_chu1" id="chinh_chu1" onchange="setCheckbox(chinh_chu1,'chinh_chu')"> 
					Bạn là môi giới?
		</span>
...
{?>
	<div id="msgErrCapt" class="errouser">Captcha sai, vui lòng nhập lại.</div>
			...
			<table>
				...
				<tr>
					...
					<td style="padding-left: 29px;text-decoration: underline;">
						<span class='clear'>
						<a href='<?php echo JRoute::_( 'index.php?option=com_user&view=remind&Itemid=19' )?>'>Quên tên đăng nhập</a>/ <a href='<?php echo JRoute::_( 'index.php?option=com_user&view=reset&Itemid=19' )?>'>mật khẩu</a>
						</span>
					</td>
				</tr>
			</table>
...
<div class="properties"><!--div big-->
	<div class="info-detail"><!-- div chua title-->
        <div class="info-structure">
	        ...
	        echo $this->propertyData['loai_giao_dich']." ".$this->propertyData['loai_bds']." ".$da.", ".$this->propertyData['quan_huyen'].", ".$this->propertyData['tinh_thanh'];?>
	        <div class='class1' style="float:right;font-weight: normal;text-transform: capitalize;"> 
							<li class='datalist'>
								Mã số:
							</li>
			...
			?>
			<?php if(empty($info1['start']) && empty($info2['start']) && empty($info3['start'])) {
				echo "<div style='float:left;line-height:15px'>Hiện tin này không có lệnh đẩy/đánh dấu/nổi bật</div>";
			}	
			?> 
			<table border="0" style="width:450px;float:left; margin-left:-3px;"> 
				<?php if (isset($info1['start'])) {?><tr><td>Đã được hẹn giờ đẩy tin lúc <b><?php echo date("H:i d-m-Y",$info1['start']); ?></td></tr><?php }?></b>
				<?php if (isset($info2['start'])) {?><tr><td>Đã được hẹn giờ đánh dấu lúc <b><?php echo date("H:i d-m-Y",$info2['start']); ?></b> đến <b><?php echo date("H:i d-m-Y",$info2['end']); ?></b></td></tr><?php }?>
				<?php if (isset($info3['start'])) {?><tr><td>Đã được hẹn giờ nổi bật lúc <b><?php echo date("H:i d-m-Y",$info3['start']); ?></b> đến <b><?php echo date("H:i d-m-Y",$info3['end']); ?></b></td></tr><?php }?>
				<tr> 
...
?>
	...
	<?php if($this->status==0){?>
						...
						<div class='clas2'> <!-- loại hình giao dịch-->
							...
							<li class='datalist-detail'>
								<?php if($this->propertyData['du_an']=='Vui lòng chọn') echo '_'; else echo $this->propertyData['du_an']?>
							</li>
						</div> <!-- loại hình giao dịch-->
				...
				<li class='datalist-detail'>
					<?php
					  ...
					  else
					  {
					  	...
					  	<input id="dien_tich_su_dung" type="text" name="dien_tich_su_dung" value="<?php if(isset($_SESSION['tmp'])) echo $_SESSION['tmp']['dien_tich_su_dung']; else echo $this->propertyData['dien_tich_su_dung'] ?>" class="numberbox livinguse admin-input" size="7" onChange="getonchangevalue(this.value,'divliving_space','m<sup>2',1)"/>m<sup>2</sup>
			...
			<div class='class1'> <!-- diện tích sàn-->
				...
				<?php
					  ...
					  else
					  {
					  	...
					  	<input id="dien_tich_khuon_vien" type="text" name="dien_tich_khuon_vien" value="<?php if(isset($_SESSION['tmp'])) echo $_SESSION['tmp']['dien_tich_khuon_vien']; else echo $this->propertyData['dien_tich_khuon_vien'] ?>" class="numberbox livinguse admin-input" size="7" onChange="getonchangevalue(this.value,'divliving_space','m<sup>2',1)"/>m<sup>2</sup>
			...
			<div class='clas2'>
				<ul>
					<li class='back' onclick="javascript:history.go('<?php if(isset($_SERVER['HTTP_REFERER'])) echo "-1"; else echo JURI::base() ?>')">
					Quay lại
					</li>
...
<div class="info-structure" class='chitiet-mlbds'>
				<div class='info-structure-title'>
		         ...
		         else
		         {
					...
					echo $this->editor->display('mo_ta_chi_tiet', $this->propertyData['mo_ta_chi_tiet'] , '99%', '300', '75', '20', false ) ;				
					echo "</div>";
		         }
		         ?>
			...
			<div class='info-contact'>
				...
				<div class='details-contact'>
					<ul>
						...
						<li>
							<h4 class='componentheading'><a class='btn-search show_hide'href="javascript:;">Gửi tin nhắn</a></h4>
							<script>
...
<script>
	jQuery(function(){
		...
		var valt = $("#du_an_id :selected").html();
		$("#du_an_text_value").val(valt);
		$("#du_an_id").change(function(){
			$("#du_an_text_value").val($("#du_an_id :selected").html());
		});
		var unit = $("select#kind_id option:selected").val();
