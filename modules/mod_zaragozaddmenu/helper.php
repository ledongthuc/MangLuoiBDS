 ...
 * @since		1.5
 */
class modZaragozaDDMenuHelper
{
	function buildXML(&$params)
	{
		$menu = new JZDDMenuTree($params);
		$items = &JSite::getMenu();
	...
	function &getXML($type, &$params, $decorator)
	{
		...
		if (!isset($xmls[$type])) {
			$cache =& JFactory::getCache('mod_zaragozaddmenu');
			$string = $cache->call(array('modZaragozaDDMenuHelper', 'buildXML'), $params);
			$xmls[$type] = $string;
		}
 ...
 * @since		1.5
 */
class JZDDMenuTree extends JTree
{
	/**
	...
	{
		$this->_params		=& $params;
		$this->_root		= new JZDDMenuNode(0, 'ROOT');
	...
	function addNode(&$params, $item)
	{
		...
		$node = new JZDDMenuNode($item->id, $item->name, $item->access, $data);
 ...
 * @since		1.5
 */
class JZDDMenuNode extends JNode
{
	/**
	 * Node Title
	 */
