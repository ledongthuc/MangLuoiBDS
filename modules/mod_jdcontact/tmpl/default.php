...
if($_POST)
{
	...
	if ( $ten == "Tên:" )
	{
		$result = "".JText::_('họ tên không được để trống')."";
	}
	elseif (!preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/", $email))
	{
		$result = "".JText::_('email không chính xác')."";
	}
	
	else if($phno=="Điện thoại:")
	{
		$result = "".JText::_('số điện thoại không được để trống')."";
	}
	...
	$result = "".JText::_('điện thoại chỉ nhập số').""; 
		...
		{
			$result = "".JText::_('nội dung không được để trống')."";
		}
