...
$db->setQuery('SELECT * FROM `jos_push` WHERE id<>'.$_POST['item'].' AND user='.$user->id.' ORDER BY start ASC'); 
$db->query();  
$info=$db->loadObjectList();
...
<div class="componentheading">SỬA HẸN GIỜ</div>
	  ...
	  echo '
		...
		<td><input readonly type="text" name="editd" size="20" id="Date_edit" <?php if (($info['end']>$time) &&($info['start']<$time)) 
	          echo ''; else echo 'onclick="calendar(\'edit\')"'; ?> value="<?php echo date("d-m-Y",$info['start']);?>" width="100px"/>
		<input <?php if (($info['end']>$time) &&($info['start']<$time)) 
		...
		<td><input readonly type="text" name="editd2" size="20" id="Date_edit2" onclick="calendar('edit2')" value="<?php echo date("d-m-Y",$info['end']);?>" width="100px"/>
		<input readonly type="text" name="edith2" size="5" id="hour2x" value="<?php echo date("H",$info['end']);?>" width="100px"/>h
		<input readonly type="text" name="editm2" size="5" id="min2x" value="<?php echo date("i",$info['end']);?>" width="100px"/>
		</td> 
		</tr> 
		</table>
