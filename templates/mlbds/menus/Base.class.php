...
$(document).ready(function(){
	//$(".classmenu li:last a").css("background","none");
	jQuery(".classmenu li:last a").addClass('menuend');
	jQuery(".classmenu li:last").addClass('menuend1');
	var id=<?php echo JRequest::getVar('Itemid')?>;
	if(id=='10'){
		jQuery("#menu290").addClass('active');
	}
	if(id=='31'){
		jQuery("#menu24").addClass('active');
		jQuery("#Mod249").css('display','none');
	}
	else if(id!='31'){
		jQuery("#Mod253").css('display','none');
		jQuery("#Mod213").css('display','none');
	}
	if(id=='218'){
		jQuery("#menu290").addClass('active');
		jQuery("#Mod253").css('display','block');
		jQuery("#Mod213").css('display','none');		
		jQuery("#Mod249").css('display','none');
	}
	<?php 
   				...
   				if($user->id !=0){
   			?>
   					jQuery("#dangtin").css("display","none");
   					jQuery("a#menu16").attr('href','<?php echo JURI::base()."vi?option=com_u_re&view=manage&layout=form&Itemid=224"?>');
   					jQuery(".item6 a").attr('href','<?php echo JURI::base()."vi?option=com_u_re&view=manage&layout=form&Itemid=224"?>');
   					jQuery("a#menu196").attr('href','<?php echo JURI::base()."vi?option=com_u_re&view=manage&layout=yeucau&Itemid=242"?>');
					<?php if($_GET['Itemid']==224){?>   					
   						jQuery(".item6").addClass('active');
   					<?php }?>
   					<?php 
//   					if ( !empty( $_GET['debug'] ) )
//   					{
...
//   					}  
   					if($_GET['Itemid']==242)
   					{?>   					
						jQuery("a#menu196").parent().addClass('active');
						jQuery("a#menu196").addClass('active');
					<?php }?>
   			...
   			<?php 
					if( $_GET['Itemid']==245 || $_GET['Itemid']==246 || $_GET['Itemid']==247)
					{?>   					
					jQuery("a#menu196").parent().addClass('active');
					jQuery("a#menu196").addClass('active');
				<?php }?>
