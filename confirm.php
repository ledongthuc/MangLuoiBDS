	...
	if ($data['user']!=0) {     
		...
		for ($i=1;$i<=$dema;$i++)
			if ($manga[7][$i]!=1) {
				...
				if ($dstart==$time) {
					$cmd="UPDATE `iland4_bat_dong_san_vi` SET ngay_chinh_sua=".$time." WHERE id=".$data['post'];
					@mysql_query($cmd); 
				} 
			   	$cmd='INSERT INTO `jos_push` VALUES (0,'.$data['post'].','.$data['user'].','.$dstart.','.$dend.','.$type.','.$status.','.time().',0,0)'; 	
				@mysql_query($cmd);
		}	
		for ($i=1;$i<=$demb;$i++)
			if  ($mangb[7][$i]!=1) {  	 
