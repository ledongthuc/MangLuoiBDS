...
<!-- <script type="text/javascript" src="<?php echo JURI::root()?>templates/mlbds/js/jquery-1.4.4.js"></script> -->
	...
	function showMailBox(){				
		jQuery('#showbox').bPopup();
		jQuery("#sendmail").click(function(){
			var mailfrom 	= 	jQuery("#mailfrom").val();
			var mailto 		= 	jQuery("#mailto").val();
			var subject 	=	jQuery("#subject").val();
			var content 	=	jQuery("#content").val();
			var input_data = '&mailfrom='+mailfrom+'&mailto='+mailto+'&subject='+subject+'&content='+content;
			jQuery.ajax({
					   type: "POST",
					   url:  "<?php echo JURI::base().'modules/mod_social/tmpl/email/sendmail.php'?>",
					   data: input_data,
					   success: function(msg){
						   jQuery('.loading-img').remove();
						   jQuery('<div class="message">').html(msg).appendTo('div#result').hide().fadeIn('slow');
						   jQuery("#mailfrom").val("Email của bạn");
						   jQuery("#mailto").val("Email người nhận");
						   jQuery("#subject").val("Tiêu đề thư");
						   jQuery("#content").val("Nội dung thư");
					   }
					});
				return false;
		    });
...
<a class="fancybox" onclick="showMailBox()" style="cursor: pointer;"><span>Email</span></a>
<div id="showbox" class='facebook' style="width:300px; height:430px; display: none;"><span class="bClose"></span>
		...
		<div class="pp_left fb_left ">
			<div class="pp_right fb_right">
					<div class="contact_title">
							...
							<div class='componentheading'>GỬI EMAIL</div>
								<div id="successbox"></div>
			<div class="form_detail">
							<table class="formajax">
								<tr>
									<td width="240px"><input type="text" class="textbox" id="mailfrom" autocomplete="off" onclick="if(this.value=='Email của bạn') this.value=''" onBlur="if(this.value=='') this.value='Email của bạn'" value="Email của bạn"/></td>  
									<td><div id="fromalert" class="alerttext"></div></td>
								</tr>
								<tr>
									<td>
									<input type="text" class="textbox" name="mailto" id="mailto" autocomplete="off" onclick="if(this.value=='Email người nhận') this.value=''" onBlur="if(this.value=='') this.value='Email người nhận'" value="Email người nhận"/></td>  
									<td><div id="toalert" class="alerttext"></div></td>
								</tr>
								<tr>
									<td><input type="text" class="textbox" name="subject" id="subject" autocomplete="off" onclick="if(this.value=='Tiêu đề thư') this.value=''" onBlur="if(this.value=='') this.value='Tiêu đề thư'" value="Tiêu đề thư"/></td>  
									<td><div id="subjectalert" class="alerttext"></div></td>
			</tr>
			<tr>
				<td colspan="2"><textarea class="detail_text" id="content" onclick="if(this.value=='Nội dung thư') this.value=''" onBlur="if(this.value=='') this.value='Nội dung thư'">Nội dung thư</textarea></td>
			</tr>
</table>
  
<div class="sendbutton" style="width: 245px"><center>
<input type="submit" class=" btn-search" id="sendmail" value="Gửi thư" />
</center>
</div>
</div>
			</div>
