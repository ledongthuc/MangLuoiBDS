...
<form action="index.php?option=com_users" method="post" name="adminForm">
	<table>
		<tr>
			...
			<td nowrap="nowrap">
				<?php echo $this->lists['speak'];?>
				<?php echo $this->lists['boss'];?>
				<?php echo $this->lists['report'];?>
...
<!--				end dang lam-->
		...
		<?php
			...
			for ($i=0, $n=count( $this->items ); $i < $n; $i++)
			{
				...
				$imgapp 	= $row->approved ? 'publish_x.png' : 'tick.png';
				$approved = $row->approved ? 'unapprove' : 'approve';
				$altapp 	= $row->approved ? JText::_( 'Enabled' ) : JText::_( 'Blocked' );
				$link 	= 'index.php?option=com_users&amp;view=user&amp;task=edit&amp;cid[]='. $row->id. '';
...
<!--				hoan dang lam-->
				<td align="center">				
				<a href="javascript:void(0);" onclick="return listItemTask('cb<?php echo $i;?>','<?php echo $approved ?>')">
						<img src="images/<?php echo $imgapp;?>" width="16" height="16" border="0" alt="<?php echo $altapp; ?>" /></a>
				</td>
...
<!--				end dang lam-->
				...
				<!-- TODO Add phone - chen cot de hien thi so phone////////////////-->
				<td>
						<?php echo $row->phone; ?>
				</td>
