...
$getEsQuery .= " AND jos_push.id IN ( SELECT p.id FROM jos_push p, ( SELECT `bds`, MAX( `type` ) maxType FROM jos_push p WHERE p.`start` < $currentTimeStamp AND p.`end` > $currentTimeStamp  GROUP BY bds ) p1 WHERE p.type = p1.maxType AND p1.bds = p.bds) ";
...
//if ( !empty( $_GET['searchType'] ) )
if ( $_GET['Itemid'] == 244 && !empty( $_GET['loai_giao_dich_id']))
{
...
$templatePathpa = JPATH_THEMES . DS . $currentTemplate . DS . "html" . DS . "mod_danh_sach_BDS";
...
$db1->query(); 
$totalItem=$db1->loadResult();
$totalPage = round( ( $totalItem / $limit ) );
$ajaxPaging = ilandCommonUtils::getAjaxPagination( $contentElementId, $url, $currentPage, 
											$totalPage, $templatePathpa, $ajaxPagingTemplate );
