...
function DynamicTree(id, basePath) {
    this.foldersAsLinks = false;
    this.path = basePath+"/images/";
    this.img = {
    ...
    this.init = function() {
        ...
        for (p in this.img) {
			...
			lastItemOfArray(this.imgObjects).src = this.img[p];
			
            //this.img[p] = this.imgObjects.getLast().src;
			this.img[p] = lastItemOfArray(this.imgObjects).src;
        }
    ...
    this.parse = function(nodes, tree) {
        for (var i = 0; i < nodes.length; i++) {
            ...
            if (nodes[i].nodeType == 1 && nodes[i].childNodes) {
				...
				this.parse(nodes[i].childNodes, lastItemOfArray(tree.childNodes));				
            }
        }
    };
    this.nodeClick = function(id) {
        ...
        if (el.style.display == "block") {
			...
			//prototyped function remove by Value replaced with removeFromArray to make compatible with html area xtd GT 2005/06/09 */
            //this.opened.removeByValue(id);
			this.opened=removeFromArray(this.opened,id);
        } else {
    ...
    this.loadState = function() {
        ...
        if (opened) {
			...
			/* .filter function replaced because it screwed up HTML Edtior XTD - GT 2005/06/09 */
            /*this.opened.filter(function(id) { return self.allNodes[id] && self.allNodes[id].isFolder && self.allNodes[id].childNodes.length;  });*/
			filterArray	 (this.opened, function(id) { return self.allNodes[id] && self.allNodes[id].isFolder && self.allNodes[id].childNodes.length;  });
        }
    };
    ...
    function Node(id, text, parentNode, childNodes, isDoc, isFolder) {
        ...
        this.isLast = function() {
            if (this.parentNode) {
				...
				return lastItemOfArray(this.parentNode.childNodes).id == this.id;
            }
        ...
        this.toHtml = function() {
            ...
            if (this.isFolder) {
                ...
                if (this.childNodes.length) {
                    nodeIcon = (stringInArray(self.opened,this.id) ? (this.isLast() ? self.img.nodeOpenEnd : self.img.nodeOpen) : (this.isLast() ? self.img.nodeEnd : self.img.node));
                } else {
                    nodeIcon = (this.isLast() ? self.img.leafEnd : self.img.leaf);
                }
                var icon = ((stringInArray(self.opened,this.id) && this.childNodes.length) ? self.img.folderOpen : self.img.folder);
                ...
                if (this.childNodes.length) {
                    ...
                    if (stringInArray(self.opened,this.id)) {
...
/* THIS SCRIPT SCREWS UP HTML AREA XTD SO I'VE REPLACED IT!! (GT 2005/06/09) */
...
*/
function removeFromArray(inArray, value) {
	if (!inArray){return;}
	var i, indexes = [];
	for (i = 0; i < inArray.length; ++i) {
		if (inArray[i] === value) { indexes.push(i); }
	}
	for (i = indexes.length - 1; i >= 0; --i) {
		inArray.splice(indexes[i], 1);
	}
	return inArray;
};
...
*/
function filterArray (inArray, func){
	var i, indexes = [];
	for (i = 0; i < inArray.length; ++i) {
		if (!func(inArray[i])) { indexes.push(i); }
	}
	for (i = indexes.length - 1; i >= 0; --i) {
		inArray.splice(indexes[i], 1);
	}
