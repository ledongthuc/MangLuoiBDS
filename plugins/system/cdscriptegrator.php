        	...
        	if ($this->enableLibrary($library)) {
	        	...
	        	if (JFile::exists($library_class_path)) {
	        		...
	        		if (is_callable(array($class_name, 'importFiles'))) {
	        			...
	        			if (is_array($files_to_import)) JScriptegrator::library($library, $files_to_import);
	        		}
	        		...
	        		if (is_callable(array($class_name, 'scriptDeclaration'))) {
	        			...
	        			if ($script_declaration) $document->addScriptDeclaration($script_declaration);
	        		}
	        		
		        	if (is_callable(array($class_name, 'load'))) call_user_func(array($class_name, 'load'));
	        	}
        	}
