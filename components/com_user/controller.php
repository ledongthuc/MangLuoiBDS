...
class UserController extends JController
{
	...
	function save()
	{
		...
		$post['username']	= JRequest::getVar('email', '', 'post', 'email');
		...
		if($_GET['Itemid']== 81){
			$return = "index.php?option=com_user&task=edit&Itemid=81&lang=vi";
		}
	...
	function desession(){
		...
		unset($_SESSION['b']);
		unset($_SESSION['c']);
		unset($_SESSION['danhdau']);
		unset($_SESSION['daytin']);
		unset($_SESSION['noibat']);
		unset($_SESSION['mark']);
		unset($_SESSION['www']);
		unset($_SESSION['user']);
		unset($_SESSION['post']);
		unset($_SESSION['price']);
		unset($_SESSION['ddgia1']);
		unset($_SESSION['ddgia2']);
		unset($_SESSION['ddgia3']);
	}
	...
	function login()
	{
		...
		else
		{
			...
			if ( ! $return ) {
				$return	= 'index.php?option=com_user&view=login&Itemid=19';
			}
			

			// Redirect to a login form
			//$mainframe->redirect( $return );
			$mainframe->redirect( 'index.php?option=com_user&view=login&Itemid=19&m=fail' );
		}
	}
...
//		echo"<script>alert('Không có online ')</script>";
		...
		//	echo "set view = register";
		//	exit;
		} else {
			$this->setredirect('index.php?option=com_user&task=edit&Itemid=19',JText::_('You are already registered.'));
		}
	...
	function register_save()
	{
		...
		if($user->chinh_chu == 'on'){
			$user->chinh_chu = 0;
			$user->set('usertype', 'Mô giới');
			$user->set('gid', '34');
		}else{
			$user->chinh_chu = 1;
			$user->set('usertype', 'Chỉnh chủ');
			$user->set('gid', '35');
		}
		if($user->speak_english == 'on'){
			$user->speak_english = 1;
		}
		if($user->nhan_mail == 'on'){
			$user->nhan_mail = 1;
		}
		...
		if ( $useractivation == 1 ) {
			...
			$msgVar = "&msg=REG_COMPLETE_ACTIVATE";
		} else {
			$message = JText::_( 'REG_COMPLETE' );
			$msgVar = "&msg=REG_COMPLETE";
		}
		
		$this->setRedirect('index.php?option=com_user&Itemid=19&task=completeregister' . $msgVar, $message);
	}
	...
	function requestreset()
	{
		...
		if ($model->requestReset($email) === false)
		{
			...
			$this->setRedirect('index.php?option=com_user&view=reset&Itemid=19', $message);
			return false;
		}
		//$message = JText::_('PASSWORD_RESET_SUCCESS');
		$this->setRedirect('index.php?option=com_user&view=reset&layout=confirm&Itemid=19');
	}
	...
	function confirmreset()
	{
		...
		if ($model->confirmReset($token) === false)
		{
			...
			$this->setRedirect('index.php?option=com_user&view=reset&layout=confirm&Itemid=19', $message);
			return false;
		}
		$message = JText::_('PASSWORD_RESET_SUCCESS');
		$this->setRedirect('index.php?option=com_user&view=reset&layout=complete&Itemid=19',$message);
	}
	...
	function completereset()
	{
		...
		$message = 'rsps';
		$this->setRedirect('index.php?option=com_user&view=login&Itemid=19&mess='.$message);
	}
	...
	function remindusername()
	{
		...
		if ($model->remindUsername($email) === false)
		{
			...
			$this->setRedirect('index.php?option=com_user&view=remind&Itemid=19', $message);
			return false;
		}

		$message = JText::sprintf('USERNAME_REMINDER_SUCCESS', $email);
		$this->setRedirect('index.php?option=com_user&view=login&Itemid=19', $message);
	}
	...
	function changepassword()
	{
		...
		if ( $user->get('guest')) {
			...
			$this->setredirect('index.php?option=com_user&task=edit&Itemid=19');
		}
	...
	{
		// echo "<br><a href='" . $_SERVER["PHP_SELF"] . "'>" . JText::_("RETURN_TO_HOMEPAGE") . "</a>";
		$usersConfig = &JComponentHelper::getParams( 'com_users' );
		if (!$usersConfig->get( 'allowUserRegistration' )) {
			JError::raiseError( 403, JText::_( 'Access Forbidden' ));
			return;
		...
		if ( $user->get('guest')) {
			//JRequest::setVar('view', 'register_complete');
			$this->setredirect('index.php?option=com_user&view=login&Itemid=19&mess=Success');
		} else {
			$this->setredirect('index.php?option=com_user&task=login&Itemid=19&mess=Success');
		}
