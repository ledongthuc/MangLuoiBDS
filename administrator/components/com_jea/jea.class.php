...
class ComJea
{
	...
	function &getParams()
    {
		...
		if ( !is_object($instance) ) {
		    ...
		    $instance->def('show_googlemap', 1);
		    $instance->def('googlemap_apikey', 'ABQIAAAALNYzlYJGVjIDOmQ1AbNh7xSalDDeQhNpZXxFfaBEwbtzaPU5uRRu5ea2kr7');
    ...
    function getImagesById($id,$plan=false)
    {
		...
		if($plan)
		{
			$img = JPATH_ROOT.DS.'images'.DS.'com_jea'.DS.'images'.DS."Plan_".$id.DS.'main.jpg';
        	$img_url_base = $rootURL . 'images/com_jea/images/Plan_' . $id . '/' ;	
		}
...
//            $result['main_image']['weight'] = round(($file[7]/1024),1) ;// poid en Ko
		...
		if($plan) $dir = JPATH_ROOT.DS.'images'.DS.'com_jea'.DS.'images'.DS."Plan_".$id.DS.'secondary';
        ...
        if( JFolder::exists( $dir ) ){
         ...
         //secondaries images1
        $dir1 = JPATH_ROOT.DS.'images'.DS.'com_jea'.DS.'images'.DS.$id.DS.'secondary';
        $result['secondaries_images1'] = array();
        
        jimport('joomla.filesystem.folder');
        
        if( JFolder::exists( $dir1 ) ){
                
            $filesList = JFolder::files( $dir1 );

            $viewfilesList = array();
            foreach ( $filesList as $filename ) {

                $detail = array();
                $im = @getimagesize($dir1.DS.$filename);
                if ($im !== FALSE){
                    $detail['name'] = $filename;
                    $detail['width'] = $im[0];
                    $detail['height'] = $im[1];
                    	
                    $file = stat ($dir1.DS.$filename);
                    $detail['weight'] = round(($file[7]/1024),1) ;// poid en Ko
            ...
            $result['secondaries_images1'] =  $viewfilesList ; 
            
        }
          //secondaries images2
        $dir2 = JPATH_ROOT.DS.'images'.DS.'com_jea'.DS.'images'.DS.$id.DS.'secondary';
        $result['secondaries_images2'] = array();
        
        jimport('joomla.filesystem.folder');
        
        if( JFolder::exists( $dir2 ) ){
                
            $filesList = JFolder::files( $dir2 );

            $viewfilesList = array();
            foreach ( $filesList as $filename ) {

                $detail = array();
                $im = @getimagesize($dir2.DS.$filename);
                if ($im !== FALSE){
                    $detail['name'] = $filename;
                    $detail['width'] = $im[0];
                    $detail['height'] = $im[1];
                    	
                    $file = stat ($dir2.DS.$filename);
                    $detail['weight'] = round(($file[7]/1024),1) ;// poid en Ko
            ...
            $result['secondaries_images2'] =  $viewfilesList ; 
//             // start debug
//           echo "< link1 pre>";
//           print_r($result);
//           echo "</pre>";
//          
      ...
      //secondaries images3
        $dir3 = JPATH_ROOT.DS.'images'.DS.'com_jea'.DS.'images'.DS.$id.DS.'secondary';
        $result['secondaries_images3'] = array();
        
        jimport('joomla.filesystem.folder');
        
        if( JFolder::exists( $dir3 ) ){
                
            $filesList = JFolder::files( $dir3 );

            $viewfilesList = array();
            foreach ( $filesList as $filename ) {

                $detail = array();
                $im = @getimagesize($dir3.DS.$filename);
                if ($im !== FALSE){
                    $detail['name'] = $filename;
                    $detail['width'] = $im[0];
                    $detail['height'] = $im[1];
                    	
                    $file = stat ($dir3.DS.$filename);
                    $detail['weight'] = round(($file[7]/1024),1) ;// poid en Ko
            ...
            $result['secondaries_images3'] =  $viewfilesList ; 
	...
	/*
     * Chau: add function
     * Get realtor image by id
     * return array of data 
     */
    function getRealtorImagesById( $id )
    {
    	$image_data = array();
    	$image_path = JPATH_ROOT.DS . 'images' . DS . 'com_jea' . DS . 
    				'images' . DS . 'realtors' . DS . $id . DS . 'avatar.jpg';
    	
    	$detail = array();
        $im = @getimagesize($image_path);
        if ($im !== FALSE)
        {
	        $image_data['image']['name'] = JURI::root() . 'images/com_jea/images/realtors/' 
	        								. $id . '/avatar.jpg';
	        $image_data['image']['width'] = $im[0];
	        $image_data['image']['height'] = $im[1];
	                    
	        $image_info = stat ($image_path);
	        $image_data['image']['weight'] = round(($image_info[7]/1024),1) ;// poid en Ko	        
        }
        return $image_data;
    }
