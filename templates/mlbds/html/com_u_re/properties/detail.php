...
<div class='tieudebds'>
			...
			<?php if($this->status==0){?>
						<div class='gia1'>
				<li class='wlable<?php  if( $this->status != 0 ) echo "datalistadmin" ?>'> <!-- gia nguyen can-->
					Giá nguyên căn: 
				</li>
				...
				<span class="gia"> 
                    ...
                    ?>
						...
						<div class='gia2'>
				<li class='wlable <?php  if( $this->status != 0 ) echo "datalistadmin" ?>'> <!-- gia theo m-->
...
?>				<ul> 
			...
			?>
<div class='chitiet-r'>
<!-- liên hệ--><?php if( $this->status == 0 )
		{ ?>
 <div class="info-structure">
      	
  <!-- <div id="posfixed"  style="position:absolute; top:0px;">-->
  <div>
	    <div class="lienhe" <?php if ($this->status==0)echo "style='100px;'" ?> >
		<h4 class='tt-contact'>
					Thông tin liên hệ
				</h4>
		<div class='lienhe1'>
...
<?php 
if( $this->status == 0) // if quang cao
{
?>
<div class="info-structure">
<?php echo $this->quangcao; ?>
</div>
<?php
}
?>
...
if( $this->status == 0) // if bat dong san lien quan
{
	...
	{
	?>
	  <div class="info-structure1 bdslienquan"><!--BAT_DONG_SAN_LIEN_QUAN -->
            <div>
...
<div class='chitiet-l <?php  if( $this->status != 0 ) echo "chitiet-l2" ?>'>
	 ...
	 <div class='properties-detail-img-l'>
        <div class="smoothness"> <!-- div chua tab -->
	   		<ul id="tabheader" class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all">
				...
				<div style="margin-top: 5px;"> <input type="button" class="sb_ck3" onclick="hen_gio_non_user(<?php echo $_GET['id'];?>);" value="Ðẩy | Ðánh dấu | Nổi bật tin này"></div>
			<!-- day tin-->
...
<div class="info-structure">
			...
			<style>#hen_h{display:none}</style>
			   <table><tr><td>
		<!--	<div>Hiện tin này không có lệnh đẩy/ đánh dấu/ nội bật</div>-->
			</td></tr>
			...
			<table border="0" style="width:450px; margin-left:-3px;">
				...
				<?php if ($infoItem['type'] == 2) {?>
					<tr><td>Ðã được hẹn giờ đánh dấu lúc <b><?php echo date("H:i d-m-Y",$infoItem['start']); ?></b> đến <b><?php echo date("H:i d-m-Y",$infoItem['end']); ?></b></td></tr>
				<?php }?>
				<?php if ($infoItem['type'] == 3) {?>
					<tr><td>Ðã được hẹn giờ nổi bật lúc <b><?php echo date("H:i d-m-Y",$infoItem['start']); ?></b> đến <b><?php echo date("H:i d-m-Y",$infoItem['end']); ?></b></td></tr>
				<?php }?>
				<?php } // end foreach ?>
				<?php 
				$userid = $user->id; 
...
<div id="subtab2" class="box" style="visibility: hidden;height:0">
				...
				<div class='thongtinchitiet'>	
					<h6 class="mod-title title-29"><span>Thông tin chi tiết</span></h6>
	...
	<dl class="listing-detail-stats-main">
		...
		<dd class="listing-detail-stats-main-val">
		<?php echo $this->propertyData['ma_so'];?>
		</dd>
		<dt class="listing-detail-stats-main-key">
			Mục đích:
...
<dt class="listing-detail-stats-main-key">
		...
		<dd class="listing-detail-stats-main-val" style='width:70%;'>
			<?php 
		...
		<dt class="listing-detail-stats-main-key" class='wlable datalist <?php  if( $this->status != 0 ) echo "datalistadmin" ?>'>
			DT sử dụng: 
		</dt>
		<dd class="listing-detail-stats-main-val">
			<?php
		...
		<dt class="listing-detail-stats-main-key" class="<?php  if( $this->status != 0 ) echo "datalistadmin" ?> ">
			Mặt tiền: 
		</dt>
		<dd class="listing-detail-stats-main-val">
		<?php
		...
		<dt class="listing-detail-stats-main-key <?php  if( $this->status != 0 ) echo "datalistadmin" ?>">
			Phòng ngủ:
		</dt>
		<dd class="listing-detail-stats-main-val">
		<?php
		...
		<dd class="listing-detail-stats-main-val" style="width: 73%;">
	<?php if ( $this->propertyData['noi_that'] != '' )
		...
		<div class='clear'>
			...
			{ ?>
			<div class='class1'> <!-- loại giao dich-->
			...
			<div class='class1'> <!-- diện tích sử dụng-->
			...
			<div class='class1'> 
				...
				<input id="dien_tich_khuon_vien_rong" type="text" name="dien_tich_khuon_vien_rong" value="<?php if(isset($_SESSION['tmp'])) echo $_SESSION['tmp']['dien_tich_khuon_vien_rong']; else echo $this->propertyData['dien_tich_khuon_vien_rong'] ?>" style="width:50px;" class="numberbox livinguse admin-input" size="7" />m					  	
				<span class=''>Sâu </span> 	<input id="dien_tich_khuon_vien_dai" type="text" name="dien_tich_khuon_vien_dai" value="<?php if(isset($_SESSION['tmp'])) echo $_SESSION['tmp']['dien_tich_khuon_vien_dai']; else echo $this->propertyData['dien_tich_khuon_vien_dai'] ?>" style="width:54px;" class="numberbox livinguse admin-input" size="7" />m
				</li>
...
<div class='class1'> 
			...
			<div class='class1' style="width: 96%;"> 
				<li class='datalist datalistadmin'>
					Nội thất
				</li>
				<li class='datalist-detail'>
			...
			<div class='class1 dst' <?php if($_GET['Itemid']==226){ echo "style='display:block;'"; }else echo "style='display:none;'";?>> <!-- Chính chủ-->
				<li class='datalist'> 
				<?php if($this->status==0){?>
...
?>				<!-- TIEN ICH -->
			...
			<div id="gantruong" class='listing-detail-stats-main' > <!-- gần trường-->
				
			
				
				<dd class='listing-detail-stats-main-val' style='width:97%;'>
				<span style='border:none!important;'class='listing-detail-stats-main-key  <?php  if( $this->status != 0 ) echo "datalistadmin" ?>'>
					Gần trường: 
				</span>
					<?php 
					if( $this->status == 0)
			...
			<div  id="ganbenhvien" class='listing-detail-stats-main'>
				<dd class='listing-detail-stats-main-val' style='width:97%;'>
				<span style='border:none!important;' class='listing-detail-stats-main-key'  class='datalist <?php  if( $this->status != 0 ) echo "datalistadmin" ?>'>
					Gần bệnh viện: 
				</span>
					<?php 
					if( $this->status == 0)
			...
			<div id="gancho" class='listing-detail-stats-main'> <!-- gần sieu thị-->
				<dd class='listing-detail-stats-main-val' style='width:97%;'>
				<span style='border:none!important;' class='listing-detail-stats-main-key <?php  if( $this->status != 0 ) echo "datalistadmin" ?>'>
					Gần chợ/siêu thị: 
				</span>
...
<?php if( $this->status != 0 ){ ?>
<div class='lienheadmin thongtinchitiet'>
...
?>
		...
		<div class="dangtinadminbds_no thongtinthem">
			<div class='info-structure-title'><h4>
		...
		<table width="100%" border="0" class='contentpane'>
				...
				<tr>
					<td align="right" style='width:160px'><span id='SEO_PAGE_KEYWORDS'><?php 	echo JText::_('TU_KHOA_SEO') ; ?>:</span></td>
				...
				<tr>
					<td align="right" style='width:160px'><span id='SEO_PAGE_DESCRIPTION'><?php 	echo JText::_('DIEN_GIAI_SEO') ; ?>:</span></td>
...
<?php 
	...
	//	print_r($user);
	 ...
	 <div class='thongtinchitiet mato'>
	 <table width="100%">
	 <tr>
