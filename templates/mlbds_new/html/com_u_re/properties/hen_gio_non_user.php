...
<div id="subtab_2" class="box" class="box" style="display:none"> 					
						<div id="smsarea">
							<div class="smsbox">
								...
								<p class="p2">SMS NUP <?php echo $_GET['id'];?> gửi 8085</p>
								<p class="p3">(Một tin nhắn có trị giá 5,000 VND)</p>
							</div>
							<div class="smsbox">
								<p class="p1">Để đánh dấu tin này trong một (1) ngày, soạn tin theo cú pháp </p>
								<p class="p2">SMS NDD <?php echo $_GET['id'];?> gửi 8085</p>
							...
							<div class="smsbox">
								...
								<p class="p2">SMS NNB <?php echo $_GET['id'];?> gửi 8085</p>
...
?>
	...
	tableadd=function (idt,title,d,n,mark){
		var div='tb'+idt; 
		var code1='<table><th width="6px">#ID</th><th width="40">Mã tin</th><th>Hẹn giờ '+title+'</th><th width="50px">Số lượng</th><th width="75px">Đơn giá (VND)</th><th width="90px">Thành tiền (VND)</th>';
		...
		for (var i=1;i<=n;i++) 
			if (d[7][i]!=1){ 
				...
				soluong+=d[4][i];
				sumrow+=sum;
			   if (idt!=1) code2+='<tr><td><input type="checkbox" id="'+div+'_'+i+'"/></td><td>'+postbds+'</td><td style="line-height:25px;">Từ ngày '+d[1][i]+' lúc '+d[2][i]+' giờ '+d[3][i]+' phút<br/>Đến ngày '+d[8][i]+' lúc '+d[2][i]+' giờ '+d[3][i]+' phút</td><td>'+d[4][i]+' ngày</td><td>'+fmoney(d[5][i])+'</td><td>'+fmoney(sum)+'</td></tr>'; else
		...
		var sumpart='<table style="margin-top:-5px; margin-bottom:5px;"><tr><td width="260px" style="text-align:right">Tổng cộng</td><td width="202px">'+soluong+' x '+fmoney(dongia)+'</td><td><div id="sumrow_'+idt+'">'+fmoney(sumrow)+'</div></td></tr></table>';  
