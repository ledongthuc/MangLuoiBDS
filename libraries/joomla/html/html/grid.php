...
class JHTMLGrid
{
	...
	*/
	...
	function newid( $rowNum, $recId, $janame, $checkedOut=false, $name='cid' )
	{
		if ( $checkedOut ) {
			return '';
		} else {
			return '<input type="checkbox" id="'.$janame.$rowNum.'" name="'.$name.'[]" value="'.$recId.'" onclick="isChecked(this.checked);" />';
		}
	}
...
//		exit;
	...
	function published_C( &$row, $i, $imgY = 'tick.png', $imgX = 'publish_x.png', $prefix='' )
	{
		$img 	= $row[8] ? $imgY : $imgX;
		$task 	= $row[8] ? 'unpublish' : 'publish';
		$alt 	= $row[8] ? JText::_( 'Published' ) : JText::_( 'Unpublished' );
		$action = $row[8] ? JText::_( 'Unpublish Item' ) : JText::_( 'Publish item' );
