...
<div class="properties"><!--div big-->
	 ...
	 <div class='properties-detail-img-l'>
		...
		<!--	<div>Hiện tin này không có lệnh đẩy/ đánh dấu/ nội bật</div>-->
				...
				<tr><td><div> <input type="button" class="sb_ck3" onclick="hen_gio_non_user(<?php echo $_GET['id'];?>);" value="Ðẩy | Ðánh dấu | Nổi bật tin này"></div></td></tr>
				</tr>
				</table>
			<?php	
			} else {	
			...
			<table border="0" style="width:450px; margin-left:-3px;">
				...
				<?php if ($infoItem['type'] == 2) {?>
					<tr><td>Ðã được hẹn giờ đánh dấu lúc <b><?php echo date("H:i d-m-Y",$infoItem['start']); ?></b> d?n <b><?php echo date("H:i d-m-Y",$infoItem['end']); ?></b></td></tr>
				<?php }?>
				<?php if ($infoItem['type'] == 3) {?>
					<tr><td>Ðã được hẹn giờ nổi bật lúc <b><?php echo date("H:i d-m-Y",$infoItem['start']); ?></b> d?n <b><?php echo date("H:i d-m-Y",$infoItem['end']); ?></b></td></tr>
				<?php }?>
				<?php } // end foreach ?>
				<?php 
				$userid = $user->id; 
				...
				<?php } ?>
				<tr> 
					<td><div> <input type="button" class="sb_ck3" onclick="hen_gio_non_user(<?php echo $_GET['id'];?>);" value="Ðẩy | Ðánh dấu | Nổi bật tin này"></div></td>				
				</tr>
			</table>
			<?php } // end else count ?> 
			<!---------------------------------------->
...
?>				<!-- TIEN ICH -->
			...
			<div  id="ganbenhvien" class='listing-detail-stats-main'>
				<dt class='listing-detail-stats-main-key'  class='datalist <?php  if( $this->status != 0 ) echo "datalistadmin" ?>'>
					Gần bệnh viện: 
				</dt>
				<dd class='listing-detail-stats-main-val' style='width:70%;'>
					<?php 
