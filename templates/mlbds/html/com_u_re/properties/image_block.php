...
if ( (!empty( $this->data ) && is_array( $this->data )) || $status != 0 )
{
	...
	?>
	<a id='' rel='prettyPhoto[image_bds]' onclick="return false" href='<?php echo $mainImage['max_url'] ?>' title='<?php echo $slideshowTitle?>' >
	...
	{
	?>
	<a rel='prettyPhoto[image_bds]' onclick="return false" href='<?php echo JURI::root()?>images/noimage.jpg'  >
...
?><div class="snd_imgs" ><!--hình ảnh thumbsnail-->
 ...
 ?>
 	...
 	onclick="swapImage('img_preview','<?php echo $mainImage['preview_url'] ?>', '<?php echo $mainImage['max_url'] ?>')" />
	 	...
	 	?>
            ...
            .'&amp;controller=properties&amp;task=deleteimg&amp;id='. $id?>" ><?php echo JText::_('DELETE') ?></a>	
	 	<?php	
	 	}
	 	?>
 ...
 ?>
  ...
  <img src="<?php echo $image['min_url'] ?>"  alt="<?php echo $image['name'] ?>"
   ...
   <a rel='prettyPhoto[image_bds]' href='<?php echo $image['max_url'] ?>' style='display:none' title='<?php echo $slideshowTitle?>'> </a>
    ...
    <a href="<?php echo JURI::base() . 'index.php?option=com_u_re'
           .'&amp;controller=properties&amp;task=deleteimg&amp;id='.$id.'&amp;image='.$image['name']?>" ><?php echo JText::_('DELETE') ?></a>	   
    <?php 
	 	}
    ?>
...
<span class="input_hinhanhchinh_dangtin" >
  <input id='main_image' type="file" name="main_image" value=""  size="30"/><input style="
    float: right;"class='button' type="button" value="Reset" onclick='resetImage(20)'>
  </span>
</div><!--  ANH_CHINH -->
...
<script type="text/javascript" charset="utf-8">
	...
	{
		$("a[rel^='prettyPhoto']:eq(0)").trigger('click.prettyphoto');
		//return false;
	 }	
</script>
<div class="clear"> </div>
<div class="clear"> </div>
<input id='anh_tmp' type="hidden" name="anh_tmp" value=""  size="30"/>
<script>
jQuery(function(){
	jQuery("#main_image").change(function(){
		var temp = jQuery(this).val();
		jQuery('#anh_tmp').val(temp);
	});
})
</script>
