...
<?php }?>
	...
	<div>
		<ul>
			...
			<li style="width: 50%;" class='mucgiataoyeucau'>
				
				<span class='clear'>
						<div style='float:left;padding:0;height: 21px;width: 183px;'>
			...
			<li style="margin-top:13px;width:122px;margin-bottom:0">
					<span class='clear check'>
			...
			<li>
				...
				<span>
					<select id="tinh_trang_noi_that" name="tinh_trang_noi_that" class='sel-tyc'>
							<option value="-1" <?php if ( !isset ( $_SESSION['tmp']['tinh_trang_noi_that'] ) || $_SESSION['tmp']['tinh_trang_noi_that'] == -1 ) {?> selected <?php }?>>Bất kỳ</option>
							<option value="1" <?php if ( isset ( $_SESSION['tmp']['tinh_trang_noi_that'] ) && $_SESSION['tmp']['tinh_trang_noi_that'] == 1 ) {?> selected <?php }?>>Có</option>
							<option value="0" <?php if ( isset ( $_SESSION['tmp']['tinh_trang_noi_that'] ) && $_SESSION['tmp']['tinh_trang_noi_that'] == 0 ) {?> selected <?php }?>>Không</option>
					</select>
				</span>
			</li>
	...
	<input type="text" id="email" name="email" class="input-tyc" size="30" value="<?php if(isset($_SESSION['tmp'])) echo $_SESSION['tmp']['email'];?>" style="width: 160px!important;margin-left:0px" maxlength="100"  onBlur="KiemTraEmail(this.value,'msgErrEmail','<?php echo JURI::base() ?>')"/><div id="msgErrEmail" class="errouser"><input type="hidden" name="emailFail" id="emailFail" value="0"  /></div>
	<br />
	</div>
	</div>
	<?php }?>
...
<div class='clear' >
<center>
	<input type="button" class="btn-search" value="Tạo yêu cầu" onclick="renderSQL()" />
...
function renderSQL(){
	...
	{
		condition += " AND duong_pho=" +"'"+ $("#duong_pho").val()+"'";
	}
	...
	if(itemid == 245){
		...
		if(document.getElementById('email').value==''){
			alert('Vui lòng nhập Email để chúng tôi có thế gửi báo cáo yêu cầu bất động sản của bạn');
